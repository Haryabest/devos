...
function buildPayload(store: SyncStoreName): unknown {
  switch (store) {
    ...
    case 'whiteboard':
      return scopeWhiteboards(useWhiteboardStore.getState().boards);
  }
}
  ...
  try {
    switch (msg.store) {
      ...
      case 'clients':
        ...
        break;
      case 'whiteboard':
        useWhiteboardStore.getState().mergeRemoteBoards(msg.payload as import('@/shared/types/whiteboard').WhiteboardData[]);
        break;
    }
  } finally {
...
function syncCollabRooms() {
  const { syncRooms, isJoinedProject } = useTeamStore.getState();
  syncRooms.forEach((projectId) => {
    joinCollabRoom(projectId, isJoinedProject(projectId) ? 'guest' : 'host');
  });
}

function handleHostLeft(projectId: string) {
  if (!useTeamStore.getState().isJoinedProject(projectId)) return;
  leaveCollabRoom(projectId);
  useTeamStore.getState().detachJoinedProject(projectId);
  window.dispatchEvent(new CustomEvent('devos:host-left', { detail: { projectId } }));
}
...
export function requestCollaborationSync() {
  ...
  const stores: SyncStoreName[] = ['projects', 'tasks', 'docs', 'api', 'team', 'whiteboard'];
...
export function initCollaborationSync() {
  ...
  connectCollabWs(
    ...
    ({ projectId }) => handleHostLeft(projectId),
  );
  subscribeCollabWsStatus((connected) => {
    notifyListeners();
    if (connected) syncCollabRooms();
  });
  ...
  useTeamStore.subscribe((state, prev) => {
    if (state.syncRooms === prev.syncRooms) return;
    const added = state.syncRooms.filter((id) => !prev.syncRooms.includes(id));
    const removed = prev.syncRooms.filter((id) => !state.syncRooms.includes(id));
    added.forEach((id) =>
      joinCollabRoom(id, state.isJoinedProject(id) ? 'guest' : 'host'),
    );
    removed.forEach((id) => leaveCollabRoom(id));
  });
  ...
  useWhiteboardStore.subscribe((state, prev) => {
    if (applyingRemote || state.boards === prev.boards) return;
    debouncedBroadcast('whiteboard');
  });
}
